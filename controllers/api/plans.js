...
*/
  ...
  try {
    ...
    if (req.session.loggedIn) {
      ...
      var dbActivityData1 = dbActivityData.get({ plain: true });
      res.status(200).json(dbActivityData1);
      console.log({ plan_post_message: "Activity saved successfully" });
    } else {
      console.log({
        plan_post_message: "Please login in order to save the plan",
      });
      res.status(404).end();
    }
  } catch (err) {
...
*/
  ...
  try {
    ...
    if (req.session.loggedIn) {
      ...
      var dbHotelData1 = dbHotelData.get({ plain: true });
      res.status(200).json(dbHotelData1);

      console.log("Keerti Bewakoof#2");
      console.log(dbHotelData1);
...
*/
  ...
  try {
    ...
    if (req.session.loggedIn) {
      ...
      var dbRestaurantData1 = dbRestaurantData.get({ plain: true });
      res.status(200).json(dbRestaurantData1);
